    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 xl:h-20">
          {/* Logo */}
          <a href="#home" className="flex-shrink-0 relative w-52 h-16 xl:w-64 xl:h-20 transition-all">
            <Image
              src="/images/logo.png"
              alt="Saboroso Logo"
              fill
          ...
          <nav className="hidden xl:flex xl:space-x-8">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                <a
                  href={link.href}
                  className="text-white hover:text-saboroso-gold text-xs xl:text-sm font-semibold tracking-wider uppercase transition-colors duration-200 py-2 flex items-center gap-1"
                >
                  {link.name}
                  {link.hasDropdown && (
                    <svg
                      className="w-3 h-3 text-white/60 group-hover:text-saboroso-gold transition-colors"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div className="hidden xl:block">
            <a
              ...
              className="inline-flex items-center gap-2 bg-saboroso-red hover:bg-saboroso-red-dark text-white px-6 py-3 rounded-full text-sm font-semibold tracking-wider uppercase transition-all duration-300 shadow-md hover:shadow-saboroso-red/30 hover:scale-105 active:scale-95 transform"
              title="Fale Conosco"
            >
              Fale Conosco
